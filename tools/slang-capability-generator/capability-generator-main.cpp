...
struct CapabilityConjunctionExpr
{
    ...
    AddCapabilityOptions joinWithNextFlag = AddCapabilityOptions::None;
    SourceLoc            sourceLoc;
};
...
struct CapabilityDefParser
{
    ...
    {
        AddCapabilityOptions addOption = AddCapabilityOptions::None;
        for (;;)
        {
            ...
            conjunction.joinWithNextFlag = addOption;

            expr.conjunctions.add(conjunction);
            addOption = AddCapabilityOptions::None;
...
struct CapabilityConjunction
{
    ...
    bool shareTargetAndStageAtom(const CapabilityConjunction& other)
    {
        ...
        // shared stage means either `nostage > stage` or `stage == stage`
...
struct CapabilityDisjunction
{
    ...
    void addConjunction(DiagnosticSink* sink, SourceLoc sourceLoc, const CapabilityConjunction& c)
    {
        ...
        for (Index i = 0; i < conjunctions.getCount();)
        {
            ...
            else
            {
                ...
                if (!removedDuplicate && conjunctions[i].shareTargetAndStageAtom(c))
                {
                    if (sink)
                    {
                        sink->diagnose(sourceLoc,
                            Diagnostics::unionWithSameAbstractAtomDisjointCapabilitySets,
                            tryGetCapabilityName(conjunctions[i].getAbstractAtom(CapabilityDef::ptrOfTarget)),
                            tryGetCapabilityName(c.getAbstractAtom(CapabilityDef::ptrOfTarget)),
                            tryGetCapabilityName(conjunctions[i].getAbstractAtom(CapabilityDef::ptrOfStage)),
                            tryGetCapabilityName(c.getAbstractAtom(CapabilityDef::ptrOfStage))
                        );
                        sink = nullptr;
                    }
                }
    ...
    void inclusiveJoinConjunction(const CapabilityConjunction& c)
    {
        if (c.isImpossible())
            return;
        for (auto& conjunction : conjunctions)
...
void calcCanonicalRepresentation(DiagnosticSink* sink, CapabilityDef* def, const List<CapabilityDef*>& mapEnumValueToDef)
{
    ...
    for (auto& c : def->expr.conjunctions)
    {
        ...
        {
            switch (c.joinWithNextFlag)
            {
            case AddCapabilityOptions::None:
            {
            ...
            {
                exprVal.inclusiveJoinConjunction(cc);
                break;
            }
            }
        }
